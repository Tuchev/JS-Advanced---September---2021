...
describe("Christmas Movie", function() {
    ...
    describe('buy moivie', () => {
        it('correct buy movie', () => {
            expect(instance.buyMovie('Thor', ['Tigar', 'Spas', 'Tigar']))
                .to.equal('You just got Thor to your collection in which Tigar, Spas are taking part!');
        });

        it('throw error movie is alredy brought', () => {
            instance.buyMovie('Thor', ['Tigar'])
            expect(() => instance.buyMovie('Thor', ['Tigar']))
                .to.throw(`You already own Thor in your collection!`);
        });
    });
